...
| `ADMIN_PASSWORD` | Production, Preview | La clave del portal de `/admin`. Mínimo 12 caracteres. **Sin ella el portal no abre para nadie**, y eso es a propósito: adentro hay datos de familias y de menores, así que la puerta falla cerrada. **Sin** `NEXT_PUBLIC`: con ese prefijo la clave viaja al navegador y la lee cualquiera. |
| `ADMIN_COOKIE_SECRET` | Production, Preview | Opcional. Con qué se firma la sesión del portal. Si no está, se firma con `ADMIN_PASSWORD`, que funciona pero ata las dos cosas: cambiar la clave echa a todo el mundo. Ponla y son independientes. |
